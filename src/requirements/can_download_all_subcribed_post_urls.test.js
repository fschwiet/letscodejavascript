...
setup.qtest(testBlock, "user can download all posts", function() {
    ...
    for(var i = 0; i < 30; i++) {
        that.rssServer.posts.push({
            ...
            postUrl: "http://trimtest.com/" + i + "?foo=bar",
            postDate: new Date()
        });
    }
    ...
    return page.open(config.urlFor("/feeds"))
        ...
        //  Force the feed contents to be downloaded
            .then(function() {
        ...
        //
        .then(function() {
            return page.open(config.urlFor("/downloadAllPosts"));
        })
        .then(function() {
